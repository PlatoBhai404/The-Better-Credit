...
@router.get("/")
@router.get("/")
def get_wallet(db: Session = Depends(get_db), current_user: str = Depends(get_current_user)):
    user = db.query(User).filter(User.email == current_user).first()
    if not user:
        raise HTTPException(status_code=404, detail="User not found")
    
    user_cards = db.query(UserCard).filter(UserCard.user_id == user.id).all()
...
# Add card to wallet
@router.post("/")
def add_to_wallet(card_id: int, db: Session = Depends(get_db), current_user: str = Depends(get_current_user)):
    user = db.query(User).filter(User.email == current_user).first()
    if not user:
        raise HTTPException(status_code=404, detail="User not found")
    ...
    existing = db.query(UserCard).filter(UserCard.user_id == user.id, UserCard.card_id == card_id).first()
    if existing:
        raise HTTPException(status_code=400, detail="Card already in wallet")
    user_card = UserCard(user_id=user.id, card_id=card_id)
    db.add(user_card)
    db.commit()
...
def remove_from_wallet(user_card_id: int, db: Session = Depends(get_db), current_user: str = Depends(get_current_user)):
    user = db.query(User).filter(User.email == current_user).first()
    user_card = db.query(UserCard).filter(UserCard.id == user_card_id, UserCard.user_id == user.id).first()
    if not user_card:
